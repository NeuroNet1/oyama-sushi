...
const Menu = () => {
  ...
  return (
    <main className="min-h-screen pt-20">
      <SEO
        title="Menu | Oyama Sushi - Sushi, Ramen, Nigiri & More"
        description="Explore our authentic Japanese cuisine menu: sushi rolls, nigiri, sashimi, ramen, futomaki, and hot dishes. Fresh ingredients and traditional recipes."
        canonicalUrl="https://www.oyamasushi.restaurant/menu"
        keywords="sushi menu, Japanese menu, sushi rolls, nigiri, sashimi, ramen, futomaki, Japanese dishes, sushi prices, authentic Japanese food, premium sushi, fresh fish, Japanese cuisine"
                  ...
                  <div className="flex space-x-2 px-4 pb-2 min-w-max">
                    {categories.map((category) => (
                      <Button
                        ...
                        className={`whitespace-nowrap text-xs px-3 py-2 min-w-fit transition-all duration-200 ${activeCategory === category.value
            ...
            <TabsContent value={activeCategory}>
              <div className={`grid gap-6 ${isMobile
                ...
                {filteredItems.slice(0, showMore ? filteredItems.length : (isMobile ? 6 : 10)).map((item, index) => (
                  <Card
                    key={index}
                    className={`overflow-hidden hover-lift animate-scale-in cursor-pointer ${isMobile ? "w-full" : ""
                      }`}
                  ...
                  >
                    <div className={`relative overflow-hidden ${isMobile ? "h-40" : "h-48"
                      ...
                      />
                      <div className={`absolute top-2 right-2 flex gap-1 ${isMobile ? "flex-col" : "flex-row"
                        }`}>
                        {item.tags.includes("chefs-pick") && (
                          <Badge className={`bg-gold text-gold-foreground ${isMobile ? "text-xs px-2 py-1" : ""
                            }`}>
                    ...
                    <CardContent className={`pt-4 pb-6 bg-transparent ${isMobile ? "px-3" : ""
                      }`}>
                      <div className="flex justify-between items-start mb-2">
                        <h3 className={`font-serif font-semibold ${isMobile ? "text-lg" : "text-xl"
                          }`}>{item.name}</h3>
                        <span className={`font-semibold text-accent ${isMobile ? "text-base" : "text-lg"
                          }`}>{item.price}</span>
                      </div>
                      <p className={`text-muted-foreground ${isMobile ? "text-xs line-clamp-2" : "text-sm"
                        }`}>{item.description}</p>
                      <p className={`color-red ${isMobile ? "text-xs mt-1" : "text-sm mt-2"
                        }`}>
